...
async function verifyOfficePackage() {
    ...
    return { validZip: rendered.buffer.subarray(0, 2).toString() === 'PK', missing, hasNotes: files.includes('ppt/notesSlides/notesSlide1.xml'), hasTransition: /<p:transition\b/.test(slide), fileCount: files.length, byteSize: rendered.buffer.length };
}
...
async function main() {
    ...
    process.stdout.write(JSON.stringify(result, null, 2) + '\n'); if (!packageCheck.validZip || packageCheck.missing.length) process.exitCode = 1;
}
