...
![版本](https://img.shields.io/badge/%E7%89%88%E6%9C%AC-0.1.174-%2310b981)
...
本版本完成 AI 自动生成 PPT 的全链路审计与稳定性优化：
- 对空表格、列行不齐表格、无有效数值图表和未知图表类型执行受控修复或安全跳过；
- 对重复 ID、越界坐标、非法样式、未知来源引用和不受支持元素做受控归一化；
- 初稿会保留有效内容并提示已安全处理的元素数量，用户可在画布或数据分析图表入口继续补充。

（详细版本变更与历史演进说明请参阅 [CHANGELOG.md](CHANGELOG.md)；本版本说明请参阅 [v0.1.174 发布记录](docs/releases/v0.1.174-AI自动生成PPT全链路审计与表格图表稳定性优化.md)。）
