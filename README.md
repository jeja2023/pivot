...
![版本](https://img.shields.io/badge/%E7%89%88%E6%9C%AC-0.1.179-%2310b981)
...
本版本完成长期记忆产品化改造：
- 聊天、Agent、流式执行和渠道入口统一使用受治理的低信任记忆上下文，修复跨入口记忆丢失和重复注入；
- 记忆写入、确认、来源撤回、遗忘抑制、个人数据擦除、版本化纠正、项目/会话范围和有效期形成一致生命周期；
- 词法与向量候选独立召回并融合排序，向量异常自动降级为词法检索，后台可补向量和建立局部 HNSW 索引；
- 记忆中心提供来源、实际使用记录、待确认候选、范围和评测用例管理；版本库提供 100 条脱敏多轮评测基准和影子排序模式。

（详细版本变更与历史演进说明请参阅 [CHANGELOG.md](CHANGELOG.md)；本版本说明请参阅 [v0.1.179 发布记录](docs/releases/v0.1.179-长期记忆产品化改造.md)。）
