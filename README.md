...
![版本](https://img.shields.io/badge/%E7%89%88%E6%9C%AC-0.1.178-%2310b981)
...
本版本完成外部 PPTX 模板导入与复用产品化：
- 管理员可预检 PPTX，查看母版/布局缩略预览、素材、字体替代和逐项兼容性报告；
- 模板采用不可变版本快照与 CAS 源文件审计，支持异步导入任务、排队取消、审核发布、下载源文件和重新解析；
- 空白创建、AI、材料转 PPT、套版与新增页面均绑定具体模板版本，复用母版、槽位、背景、受控素材、常见转场和简单入场动画；
- PPTX 导出保留可编辑对象，并写入 PowerPoint 可识别的简单动画时间线；宏、ActiveX、OLE、复杂动画和嵌入对象不会执行或宣称无损支持。

（详细版本变更与历史演进说明请参阅 [CHANGELOG.md](CHANGELOG.md)；本版本说明请参阅 [v0.1.178 发布记录](docs/releases/v0.1.178-外部PPTX模板导入与复用产品化.md)。）
