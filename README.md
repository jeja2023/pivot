...
![版本](https://img.shields.io/badge/%E7%89%88%E6%9C%AC-0.0.102-%2310b981)
...
**当前版本**：v0.0.102（第三方代码补全兼容、工具策略只读态和登录落点修复：聊天补全接口兼容 `prompt`/`suffix` 风格补全请求，普通管理员查看工具策略时不再看到编辑入口，登录表单成功后进入主页面。累计 v0.0.43–v0.0.101 涵盖基础设施收口、流式 Markdown 节流、RAG 调试可视化、会话 PDF 导出、模型路由 6 策略、工作流可视化编辑器、流式 function calling、实时 SSE、工具箱治理、知识图谱、公告治理、多轮安全加固、工具目录去重、工作流统计闭环、普通对话图表确定性渲染、工作流编排资产化、任务记录体验收口、前后端模块化治理、监控页资源说明细化、LLM 节点编排门禁、大模型结果可读化、知识库/工具箱运行前治理闭环、低可信图谱关系审核闭环、自由任务与工作流分层治理、工具级治理、数据库安全边界和配置诊断、智能体任务记录 15 条分页、DAG 详情去重、自由任务模板保存入口前置、知识库工作台顶部减重、上传弹窗化、质量诊断按需显示、知识图谱顶部工具区合并、工具箱命名收口、全局工具策略边界、模型测试权限和模型软删除修复、工具策略卡片化治理、聊天输入状态轻量化、知识库关键词兜底、工具箱限流提示修复、知识库专题库/标签范围检索，以及知识库范围选择和专题库标签二级联动）
