...
![版本](https://img.shields.io/badge/%E7%89%88%E6%9C%AC-0.0.38-%2310b981)
...
- **界面与监控优化**：修复左侧面板宽度随标题字数变化的问题，优化 GPU、今日模型 Token、慢查询与异常告警等系统监控区域的信息完整性和控件对齐。
- **文档与版本同步**：版本升级至 `v0.0.38`；`npm test` 已通过，安全测试 `70/70`。
...
**当前版本**：v0.0.38（权限与输入工具修复、组合式 MCP 服务、图表报告工作流、知识库/智能体复核与监控布局优化）
