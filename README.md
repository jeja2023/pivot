...
![版本](https://img.shields.io/badge/%E7%89%88%E6%9C%AC-0.0.94-%2310b981)
...
**当前版本**：v0.0.94（自由任务定位与工作流孵化闭环：普通入口收口为自由任务 / 快速任务，工作流入口承载编排、发布、版本、计划任务和审计复用；自由任务可生成工作流草稿；累计 v0.0.43–v0.0.93 涵盖基础设施收口、流式 Markdown 节流、RAG 调试可视化、会话 PDF 导出、模型路由 6 策略、工作流可视化编辑器、流式 function calling、实时 SSE、能力库治理、知识图谱、公告治理、多轮安全加固、工具目录去重、工作流统计闭环、普通对话图表确定性渲染、工作流编排资产化、任务记录体验收口、前后端模块化治理、监控页资源说明细化、LLM 节点编排门禁、大模型结果可读化、知识库/能力库运行前治理闭环、低可信图谱关系审核闭环，以及自由任务与工作流分层治理）
