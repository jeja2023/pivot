...
![版本](https://img.shields.io/badge/%E7%89%88%E6%9C%AC-0.1.39-%2310b981)
...
（详细版本变更与历史演进说明请参阅 [CHANGELOG.md](CHANGELOG.md)；本版本客户端自动更新、运行依赖修复和发布检查请参阅 [v0.1.36 发布记录](docs/releases/v0.1.36-生产客户端自动更新与运行依赖修复发布记录.md)）
...
- **普通聊天自动连续执行**：Web 和 Electron 的普通聊天会自动创建持久化 Agent；复杂问题可以在页面离开、刷新或切换设备后继续执行，较长消息按模型上下文预算处理，超出聊天 Agent 桥接上限时回退普通模型，完成答案或错误会回写原会话，并支持停止、批准、拒绝和继续。
- **普通聊天 Agent 开关**：管理员可在“系统设置 → 全局参数 → 智能体后台”关闭普通会话的自动连续 Agent；关闭后普通聊天仍使用原有模型流，手工 Agent、工作流和计划任务不受影响。
