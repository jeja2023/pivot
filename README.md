...
![版本](https://img.shields.io/badge/%E7%89%88%E6%9C%AC-0.0.90-%2310b981)
...
**当前版本**：v0.0.90（知识库与能力库傻瓜式闭环升级：知识库新增三步引导、拖拽上传、试问判断、来源可见和一键回聊天；能力库新增三步引导、动作示例、配置提示和中文动作名；聊天回答保留知识库/能力库状态追踪；累计 v0.0.43–v0.0.90 涵盖基础设施收口、流式 Markdown 节流、RAG 调试可视化、会话 PDF 导出、模型路由 6 策略、工作流可视化编辑器、流式 function calling、实时 SSE、能力库治理、知识图谱、公告治理、多轮安全加固、工具目录去重、工作流统计闭环、普通对话图表确定性渲染、工作流编排资产化、智能体任务记录体验收口、前后端模块化治理、监控页资源说明细化、LLM 节点编排门禁、大模型结果可读化，以及知识库/能力库普通用户使用闭环）
