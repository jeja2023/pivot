...
![版本](https://img.shields.io/badge/%E7%89%88%E6%9C%AC-0.0.32-%2310b981)
...
- **本轮优化补充**：新增文本完整性检查 `npm run check:text`，前端共享安全 HTML 逻辑抽出为 `safe-html.js`，并收紧 CSP 脚本策略。
- **智能体队列改造**：任务队列改为数据库抢占锁，支持多实例协调、锁超时恢复和运行时状态统计。
- **Schema 版本化**：补充 `schema_migrations` 表与队列锁字段，数据库升级更易追踪和判断。
- **前端静态校验加强**：前端 ESLint 将 `no-undef` 提升为错误级别，并补齐当前全局脚本所需的浏览器全局声明。
- **运行时拆分**：将智能体队列调度从主运行时中拆出，降低单文件职责密度。
