...
![版本](https://img.shields.io/badge/%E7%89%88%E6%9C%AC-0.1.161-%2310b981)
...
本版本聚焦生产知识库的权限与向量检索修复：只有超级管理员账户 `admin` 可跨用户读取常规知识库文档、图谱、数据源与索引任务；普通管理员与普通用户默认严格按所有者隔离。产品化文档仅保留所有者、明确审核协作人或显式文档授权的受控访问路径。同步修复 pgvector HNSW 部分索引的 `WITH ... WHERE` DDL 顺序，并明确 llama.cpp Embedding 容器应通过 `--ubatch-size`/`--batch-size` 处理超过默认 512-token 的单条输入。


主业务数据库现已正式收敛为 PostgreSQL；桌面授权、本机只读 SQLite 数据源、桌面 Agent 状态库和 SQLite 文件分析能力保持独立可用，不再构成主业务库兼容路径。

（详细版本变更与历史演进说明请参阅 [CHANGELOG.md](CHANGELOG.md)；本版本说明请参阅 [v0.1.161 发布记录](docs/releases/v0.1.161-生产知识库权限与向量索引修复.md)。）
