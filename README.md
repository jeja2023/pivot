...
![版本](https://img.shields.io/badge/%E7%89%88%E6%9C%AC-0.0.82-%2310b981)
...
**当前版本**：v0.0.82（普通对话图表刷新持久化：SSE 实时图表会在 assistant 消息保存前写入 `pivot-echart` 正文，刷新后可按历史消息重新渲染；后端记录并去重同轮 chart 事件，自动图表链路取消二次发射，前端兜底忽略重复图表；累计 v0.0.43–v0.0.82 涵盖基础设施收口、流式 Markdown 节流、RAG 调试可视化、会话 PDF 导出、模型路由 6 策略、工作流可视化编辑器、流式 function calling、实时 SSE、任务时间轴、能力库治理、知识库图谱、公告治理、多轮安全加固、工具目录去重、工作流统计闭环和普通对话图表确定性渲染）
