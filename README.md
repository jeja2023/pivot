...
![版本](https://img.shields.io/badge/%E7%89%88%E6%9C%AC-0.0.99-%2310b981)
...
- **治理权限边界**：普通管理员可以查看工具策略，内置系统管理员可以修改全局工具启停、风险等级和审批要求；点击卡片“编辑”可打开居中的治理弹窗，个人工具仍按所有者权限、数据库只读限制、审批策略和运行时安全检查生效。
...
**当前版本**：v0.0.99（聊天输入区知识库/工具箱状态提示移动到按钮右侧并简化为紧凑计数文案；知识库在向量服务失败时保留关键词分片并回退 FTS/LIKE 检索；工具箱区分规划模型限流和工具执行失败；累计 v0.0.43–v0.0.98 涵盖基础设施收口、流式 Markdown 节流、RAG 调试可视化、会话 PDF 导出、模型路由 6 策略、工作流可视化编辑器、流式 function calling、实时 SSE、工具箱治理、知识图谱、公告治理、多轮安全加固、工具目录去重、工作流统计闭环、普通对话图表确定性渲染、工作流编排资产化、任务记录体验收口、前后端模块化治理、监控页资源说明细化、LLM 节点编排门禁、大模型结果可读化、知识库/工具箱运行前治理闭环、低可信图谱关系审核闭环、自由任务与工作流分层治理、工具级治理、数据库安全边界和配置诊断、智能体任务记录 15 条分页、DAG 详情去重、自由任务模板保存入口前置、知识库工作台顶部减重、上传弹窗化、质量诊断按需显示、知识图谱顶部工具区合并、工具箱命名收口、全局工具策略边界、模型测试权限和模型软删除修复，以及工具策略卡片化治理和编辑弹窗体验收口）
