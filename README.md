...
![版本](https://img.shields.io/badge/%E7%89%88%E6%9C%AC-0.1.169-%2310b981)
...
本版本完成精准知识检索与受控工具访问改造：
- 建立结构化任务状态，知识库和工具规划使用当前问题与检索 query，不再直接使用完整聊天记录；
- 知识库采用元数据过滤、向量/FTS/图谱混合召回、rerank、引用可信度门控和低置信度拒答；
- 涉及制度、政策、依据或知识库但没有可靠结果时明确拒答，禁止模型用常识补答或伪造引用；
- 工具正常路由、MCP Planner 和异常回退均限制候选数量，并按任务意图优先裁剪；
- 工具输入 Schema 严格拒绝未声明字段，同时兼容合法动态 Map；
- 本机文件访问限制在授权目录白名单内，执行前拒绝绝对路径、路径穿越、ADS 和符号链接越权；
- 新增精准检索与本机目录安全回归测试。

主业务数据库现已正式收敛为 PostgreSQL；桌面授权、本机只读 SQLite 数据源、桌面 Agent 状态库和 SQLite 文件分析能力保持独立可用，不再构成主业务库兼容路径。

（详细版本变更与历史演进说明请参阅 [CHANGELOG.md](CHANGELOG.md)；本版本说明请参阅 [v0.1.169 发布记录](docs/releases/v0.1.169-PPT制作应用产品级验收收口与导出治理增强.md)。）
