...
![版本](https://img.shields.io/badge/%E7%89%88%E6%9C%AC-0.0.95-%2310b981)
![授权](https://img.shields.io/badge/%E6%8E%88%E6%9D%83-%E5%85%A8%E6%A0%88%E7%89%88-blue)

**Pivot (智枢)** 是一款面向私有化、离线化和企业内网场景的全栈 AI 对话、自由任务与工作流编排平台。系统集成多模型接入、知识库检索、能力库调用（兼容 MCP）、自由任务、工作流编排、第三方 OpenAI-compatible API、审计日志、系统监控、数据维护和企业级权限治理能力，目标是在可控环境中提供稳定、安全、可审计的 AI 工作入口。
...
- **目标拆解**：自由任务可把复杂目标拆解为多步骤任务，按需调用知识库、会话检索、最近会话、模型列表、系统健康和能力库工具。
...
- **能力包市场**：内置工具、外部 MCP Server 和数据库 MCP 连接会同步为能力包，可按用户权限启停，并影响普通对话、自由任务工具列表和 MCP 工具调用。
...
5. 新增或刷新 MCP Server、数据库连接后，重新打开或刷新自由任务工作台即可同步看到对应能力包。
...
**当前版本**：v0.0.95（任务记录默认每页 15 条、DAG 任务详情去重、快速任务保存模板入口前置；自由任务模板来自用户保存的目标、补充说明、上下文和模型参数，并按个人/共享权限进入模板库；累计 v0.0.43–v0.0.94 涵盖基础设施收口、流式 Markdown 节流、RAG 调试可视化、会话 PDF 导出、模型路由 6 策略、工作流可视化编辑器、流式 function calling、实时 SSE、能力库治理、知识图谱、公告治理、多轮安全加固、工具目录去重、工作流统计闭环、普通对话图表确定性渲染、工作流编排资产化、任务记录体验收口、前后端模块化治理、监控页资源说明细化、LLM 节点编排门禁、大模型结果可读化、知识库/能力库运行前治理闭环、低可信图谱关系审核闭环、自由任务与工作流分层治理，以及能力库工具级治理、数据库安全边界和配置诊断）
