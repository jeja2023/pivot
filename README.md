...
![版本](https://img.shields.io/badge/%E7%89%88%E6%9C%AC-0.0.98-%2310b981)
...
- **工具策略标题与统计收口**：系统设置页面标题统一为“工具策略”，工具列表行只展示工具总数，去除容易误解的“工具包”数量。
- **工具卡片平铺展示**：工具策略页去除左侧滚动工具包列表，系统工具和全局工具服务直接参考工具箱卡片样式平铺展示，卡片说明文字统一左对齐。
- **18 个工具分页**：工具策略页每页显示 18 个工具，并接入全局统一分页控件；翻页使用前端缓存，刷新、保存和启停后再同步后端状态。
- **治理弹窗居中减重**：点击卡片“编辑”后打开页面居中的治理弹窗；弹窗去除“治理面板”字样、右上角关闭按钮和重复的工具启用区域，只保留风险等级、审批要求和适用说明。
- **表单细节优化**：治理弹窗标题和标签靠左显示，风险等级下拉框高度收缩到常规控件节奏，审批开关继续使用轨道式滑块。
- **文档与验证同步**：版本升级至 `v0.0.98`，同步更新 README、使用手册、项目汇报、更新日志和版本文件；`npm run check`、`npx eslint client/chat/tool-policy.js` 和 `node tests\security-chat\rendering-streams.js` 已通过。
...
**当前版本**：v0.0.98（工具策略页改为工具卡片平铺、18 个工具分页、统一分页控件、卡片轨道开关启停、居中治理弹窗、左对齐标签与紧凑风险下拉框；累计 v0.0.43–v0.0.97 涵盖基础设施收口、流式 Markdown 节流、RAG 调试可视化、会话 PDF 导出、模型路由 6 策略、工作流可视化编辑器、流式 function calling、实时 SSE、工具箱治理、知识图谱、公告治理、多轮安全加固、工具目录去重、工作流统计闭环、普通对话图表确定性渲染、工作流编排资产化、任务记录体验收口、前后端模块化治理、监控页资源说明细化、LLM 节点编排门禁、大模型结果可读化、知识库/工具箱运行前治理闭环、低可信图谱关系审核闭环、自由任务与工作流分层治理、工具级治理、数据库安全边界和配置诊断、智能体任务记录 15 条分页、DAG 详情去重、自由任务模板保存入口前置、知识库工作台顶部减重、上传弹窗化、质量诊断按需显示、知识图谱顶部工具区合并，以及工具箱命名收口、全局工具策略边界、模型测试权限和模型软删除修复）
