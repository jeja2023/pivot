...
![版本](https://img.shields.io/badge/%E7%89%88%E6%9C%AC-0.1.171-%2310b981)
...
本版本完成演示文稿协作交互系统级重构与客户端连接配置图标矢量化：
- 演示文稿协作弹窗拆分为「按单位选人」、「直接搜索用户」与「按单位批量授权」三项独立的清晰标签；
- 「按单位选人」采用双列规整 2×2 网格布局，彻底消除选项文字省略号截断瑕疵；
- 底部操作按钮规范为左侧「保存」、右侧「关闭」，并彻底修复关闭无响应与底层未捕获异常；
- 桌面端「服务器连接配置」标题栏与关闭按钮全面替换为矢量 SVG 图标，消除跨平台字体渲染不一致。

（详细版本变更与历史演进说明请参阅 [CHANGELOG.md](CHANGELOG.md)；本版本说明请参阅 [v0.1.171 发布记录](docs/releases/v0.1.171-演示文稿协作交互重构与客户端连接配置图标矢量化.md)。）
