...
![版本](https://img.shields.io/badge/%E7%89%88%E6%9C%AC-0.1.167-%2310b981)
...
本版本全面补齐“PPT 制作”全量能力、增强多人协作与评论批注，并优化个人工作台跳转与交互：
- 修复个人工作台“继续工作”列表跳转，未命名演示文稿直达 PPT 编辑器，公文、任务、会话多路精准分流；
- 个人工作台命令栏去除多余冗余文本，保持界面极简与 a11y 无障碍标准；
- 新增演示文稿评论线程（Comment Threads）与批注树，支持单页与全文批注、多级回复与解决状态；
- 新增在线 Presence 实时状态条与协作者权限管理（编辑者/评论者/查看者），实时展示在线人员与幻灯片位置；
- 补齐 AI 继续生成/单页改写/检查、模板治理审核状态机、演讲者提词器、短时令牌投屏与性能基准；
- 前端协作模块独立解耦，单文件行数治理严格受控在 1000 行阈值以内。

主业务数据库现已正式收敛为 PostgreSQL；桌面授权、本机只读 SQLite 数据源、桌面 Agent 状态库和 SQLite 文件分析能力保持独立可用，不再构成主业务库兼容路径。

（详细版本变更与历史演进说明请参阅 [CHANGELOG.md](CHANGELOG.md)；本版本说明请参阅 [v0.1.167 发布记录](docs/releases/v0.1.167-PPT制作应用全功能补齐与协作工作台优化.md)。）
...
- **AI 与模板化创作**：从主题、受众、页数、材料和风格生成可确认的大纲，再生成可编辑页面；内置 8 套主题模板和 10 类标准布局。
- **在线页面编辑**：支持页面缩略图管理、文本、图片、形状、表格、图表、拖拽定位、缩放、撤销/重做、自动保存、版本恢复和导出前检查。
- **材料与数据协同**：可提取 Word、PDF、表格、Markdown 等材料用于 AI 生成，也可从数据分析数据集创建带来源绑定的图表。
- **可追溯交付**：PPTX、PDF、PNG 均由服务端从同一版本化 IR 渲染，进入 Artifact → CAS → Rendition → Delivery 受控交付链，支持内容摘要、一次性下载令牌和审计。

### 数据分析与非破坏性数据清洗工作台

- **数据总览与数据集编辑管理**：数据总览表格支持行内编辑数据集名称与来源备注，具备完整性范围与截断标识，支持数据预览、Excel 导出与安全删除。
- **多 Tab 分步清洗工作台**：重构为 4 个职责分明的高密度紧凑 Tab（`规则编排`、`质量体检`、`影响预览`、`记录谱系`），支持缺失填充、去重、修剪、格式化和离群过滤规则，配备常用规则快捷模板。
- **谱系数据表格与非破坏性演进**：清洗执行永远基于原始数据生成新数据集并保存版本演进谱系，谱系通过结构化高密度数据表格呈现（含序号、版本徽标、前后行/列数对比、规则数和重放操作）。

### 智能体实时执行与稳定性

- 智能体任务、工作流、计划任务、通知、结果和评测面板通过 SSE 与轮询兜底实时同步，执行中的运行状态、运行信息和步骤会自动显示，无需手动刷新。
- **普通聊天连续执行**：在输入区选择“Agent 执行”后，Web 和 Electron 会创建持久化 Agent；任务可在页面离开、刷新或切换设备后继续执行，较长消息按模型上下文预算处理，超出预算或超时会尽量保留部分结果并回写原会话。
- **普通聊天双模式**：聊天输入区默认使用“普通回答”；需要多轮规划、工具调用、审批或后台恢复时，用户可主动切换为“Agent 执行”。管理员可在“系统设置 → 全局参数 → 智能体后台”全局允许或禁用聊天 Agent 执行模式，普通回答始终可用。
- **上下文与结果可靠沉淀**：普通聊天 Agent 保留最近历史、系统提示词、图片附件视觉输入、长期记忆、RAG 引用和 MCP 策略；终态结果按运行 ID 幂等写回，不会因断线或重复通知生成多条助手答案。
- SSE 连接异常时自动切换为更快的轮询，关闭智能体工作台后停止后台刷新；读取接口禁用浏览器缓存，并对并发详情请求进行顺序保护。
- `agent.content_review` 支持模型对象、模型 ID 和 `rows/data/items` 记录别名，执行前统一转换为文本模型和 `records` 输入，兼容自主任务、工作流、嵌套子工作流和流式工具调用。

### 长期记忆稳定性

- 长期记忆模型抽取失败时自动回退启发式规则；超时会进入按模型冷却，避免生产环境模型端点异常时重复等待。
- 可通过 `LONG_TERM_MEMORY_LLM_EXTRACTION_TIMEOUT_MS`、`LONG_TERM_MEMORY_LLM_EXTRACTION_MAX_TOKENS` 和 `LONG_TERM_MEMORY_LLM_FAILURE_COOLDOWN_MS` 调整抽取超时、输出上限和冷却时间。
...
- 同一任务默认最多并行处理 2 个批次，可通过 `DATA_ANALYSIS_SEMANTIC_BATCH_CONCURRENCY` 调整（上限 4，仍受模型端点并发限制）；配置每日 Token 额度的模型会自动串行，避免额度检查并发超发。
...
- **全自主 Agent 执行治理**：自主任务会按规划、观察、执行、诊断和审批阶段运行，支持断点恢复、幂等重试、Token/调用预算、风险与网络策略；涉及写入、通知或其他外部影响的操作会在执行前明确要求审批并保留审计记录。
...
- **执行轮次与模型用量治理**：最大执行轮次默认采用自动策略，标准模式 30 轮、深度模式 50 轮、审计模式 60 轮；人工配置也受对应模式上限约束，流式工具调用和 JSON 回退共享总轮次。单次任务 Token 上限累计输入与输出，达到预算时优先保留部分结果。
...
- **预算与重试**：支持步骤/工具/连续错误/风险/Token 预算、失败重试、运行超时、单工具超时和心跳检测；达到预算、超时或重复调用熔断时会保留已有结果并明确提示结果可能不完整。
...
- **数据汇总与分组能力**：工作流可用“数据汇总”对全部数据计算多个统计指标；“数据库分组计数”直接查询数据表并统计每组数量；“表格分组汇总”则按上游表格行的部门、状态等维度同时计算记录数、总额与平均值。
- **节点配置减负**：节点库默认展示常用节点，低频技术能力集中在“高级节点”；配置向导按主要字段与更多设置分层，条件和输出字段按选择联动，延时使用秒数，失败重试和技术标识默认收起。新建工作流添加第一个节点时会自动居中并放大，后续节点和已有布局保持不变。
...
- **目录 Release 与契约**：每次刷新会产生不可变工具目录版本，冻结输入/输出 Schema、风险、示例、认证 Scope 与定义摘要；破坏性变更进入审核，工作流会固定依赖的 release/digest 并在契约变化后提示重新确认。
- **连接账户与 OAuth**：连接器、个人/单位/服务账号及工具绑定独立管理；OAuth 使用授权码 + PKCE，支持 Scope、过期、刷新、撤销与只读连接测试，密钥和令牌不进入模型上下文、工具 Schema 或审计预览。
- **渐进式工具发现**：模型和用户可先通过 `tools.search` 获取少量授权工具摘要，再用 `tools.describe` 读取完整冻结契约并由 `tools.execute` 受控执行，避免大工具集耗尽上下文。
- **统一调用治理**：聊天、Agent、工作流、MCP RPC、OpenAI 兼容接口、数据分析和手动测试共用策略执行点；每次调用均按能力、连接、版本、输入/输出契约、审批、网络与数据边界判定并记录 Trace。
- **可靠性与运营**：支持持久化工具任务、取消和补充输入；按工具/连接执行并发隔离、限流、熔断与安全重试。运行页汇总成功率、延迟、失败和审批，工具评测提供 Top-1/Top-3 基线。
- **受控工具包发布**：`pivot.toolkit.json` 声明版本、协议、能力、Scope、隐私和工具清单；需通过 Manifest、RSA 签名、活跃目录契约和评测门禁后才能审核发布，详见[工具包发布与签名规范](docs/工具包发布与签名规范.md)。
...
- **治理权限边界**：工具策略页面由系统管理员专属管理（普通管理员不展示该标签，避免无权限访问后端全局能力接口）；系统管理员可以修改全局工具启停、风险等级和审批要求，并通过卡片“编辑”打开居中的治理弹窗。个人工具仍按所有者权限、数据库只读限制、审批策略和运行时安全检查生效。

### 4. 应用中心业务工作台

- **公文写作**：支持多文档公文起草、润色、局部改写、审校、批注、版本对比和多格式导出。
- **数据分析**：支持表格数据导入、字段画像、数据清洗、查询、透视、图表、数据比对、智能分析和结果导出；数据清洗会预览影响并生成保留规则与来源关系的派生数据集，不覆盖原始数据。
...
- **无模型排序增强**：同一用户对同一问题的历史反馈会以小样本收缩方式轻量调整候选排序，不会污染其他用户，也不会新增模型调用；引用同时展示可解释的检索可信度（相关度、词法命中、排序和反馈综合信号）。
- **重复文档治理**：文档索引时记录源文件 SHA-256 指纹，质量报告识别同一用户的完全重复文档，并提示未生成指纹的历史文档通过批量重建补齐。
...
# 达到单个时间片轮次上限时，是否从安全检查点自动续跑；仍受总时长、Token、工具和风险预算约束。
...
# 常驻 Agent：显式传入 residentKey 后启用，按用户做 LRU，默认最多 32 个
AGENT_RESIDENCY_MAX_ENTRIES=32
AGENT_RESIDENCY_IDLE_TTL_MS=1800000
...
npm run test:mcp:official:tools
npm run test:mcp:official:sse
```
...
真实外部链路验收（会请求已配置的模型监控 URL；未配置时只读请求模型 `/models` 目录，不发起补全；并验证 Embedding、外部 MCP 健康检查和数据库只读连通测试）：
