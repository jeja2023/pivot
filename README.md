...
![版本](https://img.shields.io/badge/%E7%89%88%E6%9C%AC-0.1.168-%2310b981)
...
本版本完成 PPT 制作应用的产品级验收收口与交付治理增强：
- 文稿库支持模板、状态、创建者、更新时间、标签、收藏以及标题/正文的组合筛选；
- AI 创建补齐时长、语言、图表、来源、必含内容和禁止内容等产品参数；
- 编辑器补齐多选、组合、复制粘贴、对齐分布、表格行列编辑、图片替换与裁剪；
- 质量检查新增字体回退、图片分辨率、来源引用与数据来源检查；
- 导出选项支持 16:9/4:3、备注、页码、来源标记、图片质量和字体策略，并修复 4:3 PPTX 导出；
- 新增图片质量元数据迁移和专项回归，主编辑模块保持在 1000 行阈值以内。

主业务数据库现已正式收敛为 PostgreSQL；桌面授权、本机只读 SQLite 数据源、桌面 Agent 状态库和 SQLite 文件分析能力保持独立可用，不再构成主业务库兼容路径。

（详细版本变更与历史演进说明请参阅 [CHANGELOG.md](CHANGELOG.md)；本版本说明请参阅 [v0.1.168 发布记录](docs/releases/v0.1.168-PPT制作应用产品级验收收口与导出治理增强.md)。）
