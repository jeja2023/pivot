...
![版本](https://img.shields.io/badge/%E7%89%88%E6%9C%AC-0.1.156-%2310b981)
...
知识库首页现以资料管理为主：筛选、摘要、质量告警和文档列表保持首屏可见；搜索问答、知识内容、局域网来源、索引任务和质量评测按需进入独立治理工作台。知识资产继续支持版本与发布生命周期、责任/验证/有效期治理、稳定引用、持久化索引任务、局域网来源同步、统一 ACL，以及黄金问题集与检索质量评测。

主业务数据库现已正式收敛为 PostgreSQL；桌面授权、本机只读 SQLite 数据源、桌面 Agent 状态库和 SQLite 文件分析能力保持独立可用，不再构成主业务库兼容路径。

（详细版本变更与历史演进说明请参阅 [CHANGELOG.md](CHANGELOG.md)；本版本说明请参阅 [v0.1.156 发布记录](docs/releases/v0.1.156-知识库治理工作台布局优化.md)。）
