...
![版本](https://img.shields.io/badge/%E7%89%88%E6%9C%AC-0.0.27-%2310b981)
...
- 自动化任务工作台已作为左侧工具入口接入，支持选择模型、输入目标、后台执行、停止任务、重新运行、查看步骤、工具调用和最终结果。
- 自动化任务记录支持“移除记录”：前台列表软删除隐藏，数据库保留删除时间、删除人和删除原因；内置 `admin` 超级管理员可通过“删除审计”入口查看被移除记录。
- MCP 作为独立左侧工具接入，支持配置个人/全局 MCP Server、刷新工具缓存，并将外部工具纳入自动化可用工具池。
- 知识库迁移为独立工作台，整合文档列表、RAG 配置、召回测试、删除审计、批量重建、批量删除、失败重试和上传文档；文档列表默认每页 15 条并带分页。
- 自动化、MCP、知识库三个工具弹窗统一加宽、统一标题说明对齐、统一红色描边关闭按钮，表格、输入框、按钮和字号对齐系统设置弹窗风格。
- 权限边界进一步收紧：只有内置用户名 `admin` 是超级管理员；普通管理员不能创建全局模型、全局 Prompt、全局 MCP 服务，也不能管理其他管理员账号。
- 普通管理员和普通用户可配置自己的对话模型与向量模型；个人配置优先，未配置时回退超级管理员维护的系统默认配置。
- RAG 向量模型请求纳入统一 Token 统计，补齐 embedding 输入/总量估算，API Key、模型和用户用量统计口径保持一致。
- 审计日志全面中文化，兼容历史英文 RAG 动作值，并补齐自动化、MCP、知识库新增功能的中文审计动作与详情。
- 主界面和自动化模型选择器统一模型能力 SVG 图标与颜色样式，补齐文本能力“文”图标。
- 当前验证命令为 `npm run verify`，覆盖语法检查、ESLint 和安全测试，测试通过 `58/58`。
...
- 第三方 API 网关补齐向量转发：持有 Pivot API Key 的客户端可通过 OpenAI-compatible `/v1/embeddings` 调用当前用户可用的 RAG 向量模型，`/v1/models` 与 API 接入管理页都会暴露已配置的 embedding 模型。
- 知识库中文文件名修复：上传入口统一规范化文件名，保留正常中文并修复 latin1 mojibake，避免知识库列表中文档名称显示乱码。
- 前端 CSP 治理推进：`client/chat` 已清除全部 `onclick=` 内联事件，模型、提示词、附件、API Key 和会话菜单均改为 `data-*` 事件委托。
...
- **自动化任务工作台**：支持把复杂目标拆解为多步骤任务，自动选择知识库、会话、模型列表、MCP 等工具执行，并保留可审计的任务记录。
- **MCP 工具接入**：支持配置外部 MCP Server、刷新工具缓存，并让自动化任务按需调用外部工具；普通用户/管理员可维护个人 MCP 服务，超级管理员可发布全局共享服务。
...
- **软删除审计治理**：聊天、附件、知识库文档和自动化任务记录均采用软删除保留策略；内置 `admin` 超级管理员可查看删除审计入口，普通用户列表自动隐藏已删除数据。
...
**当前版本**: v0.0.27 (Automation, MCP & Governance Upgrade)
