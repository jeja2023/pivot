...
![版本](https://img.shields.io/badge/%E7%89%88%E6%9C%AC-0.1.159-%2310b981)
...
本版本聚焦系统交互细节的精致打磨与高频操作稳定性：彻底解决桌面端（Electron）中在右侧对话主区域滚动时因指针状态误判导致主区域不滚、反而左侧会话记录列表滚动的故障；将 Agent 控制台与工具库中的能力目录、内置工具及权限范围（global/admin/user）全面本地化为中文，支持弹窗式快捷目录浏览，优化搜索框基准对齐与焦点自动隐退；规范模态框层级梯度（主弹窗 5600、版本 6200、新建数据源 6300、Alert 7200），解决版本弹窗被遮挡在下层的问题，并补齐全局 showAlert 与局域网数据源管理弹窗。

主业务数据库现已正式收敛为 PostgreSQL；桌面授权、本机只读 SQLite 数据源、桌面 Agent 状态库和 SQLite 文件分析能力保持独立可用，不再构成主业务库兼容路径。

（详细版本变更与历史演进说明请参阅 [CHANGELOG.md](CHANGELOG.md)；本版本说明请参阅 [v0.1.159 发布记录](docs/releases/v0.1.159-工作台交互中文化治理与对话侧栏滚轮隔离.md)。）
