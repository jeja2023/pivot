...
![版本](https://img.shields.io/badge/%E7%89%88%E6%9C%AC-0.0.84-%2310b981)
...
**当前版本**：v0.0.84（文档与版本治理：补齐 `v0.0.83` 历史记录，当前版本、package 元数据、README、项目汇报和运行时版本读取统一同步到 `v0.0.84`；累计 v0.0.43–v0.0.84 涵盖基础设施收口、流式 Markdown 节流、RAG 调试可视化、会话 PDF 导出、模型路由 6 策略、工作流可视化编辑器、流式 function calling、实时 SSE、任务时间轴、能力库治理、知识库图谱、公告治理、多轮安全加固、工具目录去重、工作流统计闭环、普通对话图表确定性渲染和工作流编排资产化）
