...
![版本](https://img.shields.io/badge/%E7%89%88%E6%9C%AC-0.1.180-%2310b981)
...
本版本完成长期记忆验收收口与评测体验升级：
- 删除来源消息会取消对应抽取任务；修订栅栏、抑制记录和真实遗忘语义共同阻断撤回后的自动重建；
- 同一事实键严格按项目/会话/用户作用域处理，模型自动候选必须携带真实用户消息和证据摘录；
- Recall@8 以最终实际注入集合计算，评测持久化检索 p50/p95、预计输入 token，模型抽取 token 进入用量账本；
- 记忆中心的检索评测改为独立工作台弹窗，提供运行指标、历史记录、空状态与编辑取消交互。

（详细版本变更与历史演进说明请参阅 [CHANGELOG.md](CHANGELOG.md)；本版本说明请参阅 [v0.1.180 发布记录](docs/releases/v0.1.180-长期记忆验收收口与评测体验升级.md)。）
