...
![版本](https://img.shields.io/badge/%E7%89%88%E6%9C%AC-0.1.181-%2310b981)
...
本版本完成 Agent 可信执行闭环与可选本机执行面：
- 任务完成由 `TaskContract`、独立验收、证据和可恢复检查点共同约束；控制指令、子任务、浏览器会话、目标/监控、产物批注和模型质量画像进入持久化治理；
- 共享工具租约、渐进工具发现、业务评测、技能激活门槛与个人工作台诊断形成可观测的质量和恢复闭环；
- 已授权 Git 工作区支持受确认的读取、修改、隔离测试、登记工作树、提交、推送和 PR，物理路径校验阻止链接逃逸，搜索阻止命令选项注入；
- Windows 桌面控制仅作用于精确匹配的前台授权窗口；实时语音只在设备端识别和本地声音可确认时启用，识别文本不写入聊天、记忆或任务记录。

（详细版本变更与历史演进说明请参阅 [CHANGELOG.md](CHANGELOG.md)；本版本说明请参阅 [v0.1.181 发布记录](docs/releases/v0.1.181-Agent可信执行闭环与W18本机执行面.md)。）
