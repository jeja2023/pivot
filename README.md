...
![版本](https://img.shields.io/badge/%E7%89%88%E6%9C%AC-0.1.158-%2310b981)
...
个人 Agent 的记忆、学习与复用体验继续收口：用户修正结果后可看到是否已进入学习队列；运行详情会解释自动匹配的个人经验，并允许暂停误匹配版本。Agent 工具调用已统一采用“搜索 → 查看契约 → 执行”渐进式发现，标准与流式模式都不会预先注入完整工具目录。新增的 Terminal Runtime 只在已配置审计镜像的 Capability Worker 时开放，在任务隔离工作区中受控运行 Node/Python 与只读项目检查；未配置时保持安全拒绝，不会开放宿主机 Shell。

主业务数据库现已正式收敛为 PostgreSQL；桌面授权、本机只读 SQLite 数据源、桌面 Agent 状态库和 SQLite 文件分析能力保持独立可用，不再构成主业务库兼容路径。

（详细版本变更与历史演进说明请参阅 [CHANGELOG.md](CHANGELOG.md)；本版本说明请参阅 [v0.1.158 发布记录](docs/releases/v0.1.158-个人Agent交互终端与渐进式工具发现.md)。）
