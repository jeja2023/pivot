...
![版本](https://img.shields.io/badge/%E7%89%88%E6%9C%AC-0.1.157-%2310b981)
...
工具库现已升级为受控连接与工具生命周期平台：工具目录按 Release 冻结输入/输出契约和兼容性差异，聊天、工作流和 MCP 调用统一经过策略检查；连接账户支持 OAuth PKCE、Scope、撤销和测试，工具调用可关联版本、连接、策略决定和 Trace。目录支持渐进式搜索与按需加载契约，运行与治理页提供调用健康、失败和审批概览，工具包可通过 Manifest、签名、评测和审核后发布。

主业务数据库现已正式收敛为 PostgreSQL；桌面授权、本机只读 SQLite 数据源、桌面 Agent 状态库和 SQLite 文件分析能力保持独立可用，不再构成主业务库兼容路径。

（详细版本变更与历史演进说明请参阅 [CHANGELOG.md](CHANGELOG.md)；本版本说明请参阅 [v0.1.157 发布记录](docs/releases/v0.1.157-工具库产品化控制面与受控连接.md)。）
