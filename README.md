...
![版本](https://img.shields.io/badge/%E7%89%88%E6%9C%AC-0.0.39-%2310b981)
...
- **全局模型可测可见**：管理员和普通用户可以测试自己可见的超级管理员全局模型并查看状态，仍不能编辑、删除或查看全局模型密钥。
...
**当前版本**：v0.0.39（权限与输入工具修复、组合式 MCP 服务、数据库 MCP 生产诊断、智能体排队治理、流式会话隔离与监控布局优化）
