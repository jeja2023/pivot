...
![版本](https://img.shields.io/badge/%E7%89%88%E6%9C%AC-0.1.160-%2310b981)
...
本版本聚焦知识库数据安全隔离与桌面端交互精确性修复：将知识库集合、文档及 RAG 资源的访问控制从"共享范围 + 单位白名单"宽泛过滤彻底收紧为严格所有者隔离（`user_id = ?`），同步将知识图谱实体、关系、提及、文档的跨用户查询权限从普通管理员（`isAdmin`）升级为超级管理员（`isSuperAdmin`），消除数据越权读取风险；精确化桌面端（Electron）侧栏滚轮侦测区域，将接管判定从整个 `.sidebar` 容器收窄到 `#session-list` 与会话标题标签两个精确元素，根治右侧消息区滚轮被误判重定向至会话列表的问题。
