...
![版本](https://img.shields.io/badge/%E7%89%88%E6%9C%AC-0.1.40-%2310b981)
...
（详细版本变更与历史演进说明请参阅 [CHANGELOG.md](CHANGELOG.md)；本版本 Agent 连续执行控制回路、预算降级和断点恢复说明请参阅 [v0.1.40 发布记录](docs/releases/v0.1.40-Agent连续执行控制回路与预算降级加固.md)）
...
- **普通聊天自动连续执行**：管理员显式开启后，Web 和 Electron 会对复杂任务创建持久化 Agent；任务可在页面离开、刷新或切换设备后继续执行，较长消息按模型上下文预算处理，超出预算或超时会尽量保留部分结果并回写原会话。
- **普通聊天 Agent 开关**：`CHAT_AUTO_AGENT_ENABLED` 默认关闭；管理员可在“系统设置 → 全局参数 → 智能体后台”开启或关闭。开启后短且无任务意图的闲聊仍走普通模型流，手工 Agent、工作流和计划任务不受影响。
...
# 普通会话是否自动创建连续 Agent：1 开启，0 关闭；系统设置保存后可即时调整
