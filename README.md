...
![版本](https://img.shields.io/badge/%E7%89%88%E6%9C%AC-0.0.256-%2310b981)
...
- **执行轮次与模型用量治理**：最大执行轮次默认采用自动策略，标准模式 20 轮、深度和审查模式 50 轮，人工值最高 50；流式工具调用和 JSON 回退共享总轮次。单次任务 Token 上限累计输入与输出，留空或填写 `0` 表示不限。
- **工作流编辑体验**：工作流名称可在编排标题旁直接修改；节点名称在完成输入并按 Enter 或离开输入框后一次保存，避免逐字提交导致焦点丢失。
...
- **新闻内容校对**：数据库查询结果可接入富文本内容校对节点，自动清理 HTML 并逐条检查标题和正文；长内容会分块处理，多条记录会分别返回问题、通过或未完成状态，并可沉淀为 Markdown 报告产物。
