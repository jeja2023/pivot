...
![版本](https://img.shields.io/badge/%E7%89%88%E6%9C%AC-0.1.183-%2310b981)
...
本版本完善实时语音与桌面端受控文件保存：
- 实时语音优先使用系统默认麦克风，设备缺失、权限拒绝或被占用时会给出可操作的中文提示；
- 麦克风成功打开后才创建实时语音会话，保持设备端识别、本地 TTS 和不上传录音的隐私边界；
- 首次“保存到本机”会引导选择并授权输出目录，创建任务前按当前登录会话验证桌面设备；
- 已过期或不支持目标格式的目录不会参与保存选择，多个授权目录按序号选择，客户端重启后继续恢复受控交付轮询。

（详细版本变更与历史演进说明请参阅 [CHANGELOG.md](CHANGELOG.md)；本版本说明请参阅 [v0.1.183 发布记录](docs/releases/v0.1.183-实时语音与本机文件交付完善.md)。）
