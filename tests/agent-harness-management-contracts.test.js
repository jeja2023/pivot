...
test('Agent 控制台提供档案、记忆、经验和技能日常入口，默认隐藏运维面板', () => {
    ...
    assert.match(partial, /Agent 控制台/);
    ...
    assert.match(partial, /data-agent-harness-nav="profile"[\s\S]*?>个人档案<\/button>/);
    assert.match(partial, /data-agent-harness-nav="memory"[\s\S]*?>记忆与隐私<\/button>/);
    assert.match(partial, /data-agent-harness-nav="learning"[\s\S]*?>经验沉淀<\/button>/);
    assert.match(partial, /data-agent-harness-nav="skills"/);
    assert.match(partial, /data-agent-harness-nav="residency"[^>]*hidden/);
    assert.doesNotMatch(partial, /data-agent-harness-nav="evolution"/);
    assert.match(partial, /data-agent-harness-section="profile"/);
    assert.match(partial, /data-agent-harness-section="memory"/);
    assert.match(partial, /data-agent-harness-section="learning"/);
    assert.match(partial, />记忆与隐私<\/strong>/);
    assert.match(partial, /id="agent-memory-console-title">我的记忆/);
    assert.match(partial, /id="agent-memory-console-list"/);
    ...
    assert.match(script, /hiddenByDefaultNavs/);
    assert.match(learning, /\/memories\?status=active/);
    assert.match(learning, /\/memories\?status=disabled/);
    assert.match(learning, /data-agent-memory-action="edit"/);
    assert.match(learning, /pivot:memory-changed/);
