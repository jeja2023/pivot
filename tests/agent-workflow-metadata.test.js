...
test('工作流中文搜索在 PostgreSQL 下可执行且把通配符视为普通字符', async () => {
    const suffix = Date.now().toString(36);
    const userInfo = sql(`
        INSERT INTO users (username, password_hash, nickname, unit, role, status, created_at)
        VALUES (?, 'hash', ?, '研发部', 'user', 'active', datetime('now', '+8 hours'))
    `).run(`workflow_search_${suffix}`, '工作流搜索测试');
    const user = { id: Number(userInfo.lastInsertRowid), role: 'user', unit: '研发部' };
    let workflow = null;

    try {
        workflow = await createAgentWorkflow(user, {
            name: `检查客户端更新_%_${suffix}`,
            description: '生产环境搜索回归',
            dagSpec: {
                nodes: [{ id: 'output', title: '输出', tool: 'workflow.output', input: { name: 'result', value: 'ok' } }]
            }
        });
        const chineseMatches = await listAgentWorkflows(user, { query: '检查客户端更新' });
        assert.ok(chineseMatches.some(item => Number(item.id) === Number(workflow.id)));

        const literalWildcardMatches = await listAgentWorkflows(user, { query: '_%_' });
        assert.ok(literalWildcardMatches.some(item => Number(item.id) === Number(workflow.id)));
    } finally {
        if (workflow) sql('DELETE FROM agent_workflows WHERE id = ?').run(workflow.id);
        sql('DELETE FROM users WHERE id = ?').run(user.id);
    }
