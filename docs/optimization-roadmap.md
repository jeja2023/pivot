...
- v0.1.4 (2026-08-19) 完成认证退出与空会话体验优化、可观测性与监控单行布局增强、全链路审计中文化与 PostgreSQL 自动备份加固：CSRF 豁免 `/auth/logout` 且前端异步等待实现单次退出；空会话恢复静默重置避免错误 Toast；监控面板列宽自适应彻底消除 PostgreSQL 标签截断；慢查询与告警条目增加时间戳并重构为单行流式布局，排除大模型正常推理耗时；全模块（知识库/知识图谱等）补齐审计日志并完成操作与详情全量中文化；`pg_dump` 备份实现 Windows 常见路径自动查找与应用 Schema 范围保护。全量 457 个 JS 检查与 ESLint 0 错误 0 警告通过。
