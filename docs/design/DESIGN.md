...
- 个人自主进化与组织受控共享：`docs/design/Pivot个人自主进化与组织受控共享实施方案.md`
  - 个人域允许在无权限扩大、无副作用的范围内自动沉淀并启用已验证经验；组织域只能通过脱敏候选、评测、审批、签名、灰度和回滚发布。
  - 用户侧使用“我的记忆、我的经验、待确认改进”三个概念；底层版本、Trace、评测和签名只在高级治理视图出现。
- 系统设置与治理：
