...
- 任务与自动化：
  ...
  - 最大执行轮次属于高级安全边界，默认按运行模式自动选择而不是使用偏小的固定值；标准模式 20 轮，深度和审查模式 50 轮，人工配置最高 50。单次任务 Token 上限独立控制累计输入和输出，留空或 `0` 表示不限。
  - 达到执行轮次或 Token 上限时必须保留已有结果并明确提示可能不完整；运行详情应区分模型“执行轮次”和持久化“执行记录”，工作流 DAG 继续以节点记录为主。
  - 工作流和节点名称允许就地编辑，但提交边界必须清晰：名称编辑完成后一次提交，不能因逐字保存导致焦点丢失。
