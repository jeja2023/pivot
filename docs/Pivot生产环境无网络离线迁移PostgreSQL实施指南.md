# （归档）智枢 (Pivot) 历史离线 SQLite→PostgreSQL 迁移与插件部署指南

> **归档状态（v0.1.155 起）**：Pivot 主业务库已收敛为 PostgreSQL，本文仅保留旧版离线一次性数据迁移设计。文中引用的 SQLite 主库快照、`migrate_sqlite_to_pg.js`、`verify_pg_migration.js` 和相关核验工具已不随当前产品交付，不能在 v0.1.155 及后续版本中直接执行。
>
> 当前离线生产部署请使用 [生产环境离线部署](生产环境离线部署.md) 和 [PostgreSQL 主库运行与升级说明](生产环境PostgreSQL主库运行与升级说明.md)。
