...
- Primary product surfaces: 聊天首页、知识库、工具箱、自由任务、工作流、应用中心、系统设置、桌面客户端
- Evidence reviewed:
  - `README.md`：项目定位、功能边界和历史演进。
  - `使用帮助.md`：普通用户、管理员和系统管理员的实际使用路径。
  - `开发规范.md`：工程落地规则、UI 复用硬约束、测试和门禁。
  - `docs/公文写作设计方案.md`：公文写作模块设计。
  - `工具箱数据接入与本机能力设计方案.md`：工具箱数据源、本机能力和执行位置设计。
...
- Non-goals:
  ...
  - 不牺牲安全审计来换取短期便利。
- Success signals:
  - 用户能清楚判断本轮回答是否使用了知识库或工具箱。
  - 用户能理解数据来源在哪里、由哪台机器执行、是否可用于自动化。
  - 管理员能快速定位模型、知识库、工具箱和任务运行问题。
...
- Primary navigation:
  - 聊天：默认入口，承载普通问答、附件、知识库开关和工具箱开关。
  - 应用中心：承载公文写作、数据分析、法规查询、OCR/PDF 等业务应用。
  - 知识库：承载资料上传、专题库/标签、召回测试、质量报告和知识图谱。
  - 工具箱：承载数据源、系统工具、个人连接、外部工具服务和治理诊断。
  - 自由任务：承载探索性多步骤任务。
  - 工作流：承载稳定流程、周期任务和可审计自动化。
  - 系统设置：承载模型、用户、参数、用量、审计、工具策略和系统状态。
- Core routes/screens:
  - 聊天主界面：会话、输入、附件、知识库/工具箱状态、回答来源提示。
  - 主工作区：知识库、工具箱、应用中心和设置等都在统一工作区中打开。
...
- Test/screenshot expectations:
  ...
  - 涉及工具箱、本机能力、上传和数据源时，必须验证权限与执行位置提示。
...
- 工具箱数据接入：`工具箱数据接入与本机能力设计方案.md`
  - 工具箱采用“数据来源 + 处理工具 + 输出动作”心智模型。
  - 明确服务器可访问数据库、服务器可访问报表目录、导入数据文件和本机能力的执行位置。
- 文档处理、OCR 与 PDF：`文档处理底座与OCR_PDF工具应用分阶段开发方案.md`
...
- [ ] 本地助手是否作为桌面客户端的一部分发布，还是提供轻量独立安装包？影响：工具箱本机能力的信息架构和安装引导。
