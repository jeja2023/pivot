...
function agentAutomationTriggerConfigFields(type, config) {
    if (type === 'webhook') return `
        <div class="agent-automation-resources-form-grid">
            <label><span>输入映射（JSON）</span><textarea class="form-input" name="inputMappingJson" rows="3" spellcheck="false" placeholder='{"订单号":"data.order.id"}'>${agentEscape(JSON.stringify(config.inputMapping || {}, null, 2))}</textarea><small>键为工作流输入名，值为 Webhook 字段路径；留空传入完整请求体。</small></label>
            <label><span>固定输入（JSON）</span><textarea class="form-input" name="staticInputsJson" rows="3" spellcheck="false" placeholder='{"来源":"ERP"}'>${agentEscape(JSON.stringify(config.staticInputs || {}, null, 2))}</textarea><small>每次触发时自动附加到工作流的固定输入。</small></label>
        </div>
        <div class="agent-automation-resources-form-grid">
    ...
    if (type === 'file') return `
        ...
        <label><span>运行目标说明</span><input class="form-input" name="goalTemplate" type="text" maxlength="2000" value="${agentEscapeAttr(config.goalTemplate)}" placeholder="例如：处理新落地的报告文件"></label>`;
    return `
        ...
        <label><span>增量查询 SQL</span><textarea class="form-input" name="query" rows="3" required maxlength="4000" spellcheck="false" placeholder="SELECT * FROM orders WHERE updated_at > '{{watermark}}'">${agentEscape(config.query)}</textarea><small>必须是只读查询，并包含 <code>{{watermark}}</code> 占位符。</small></label>
        <div class="agent-automation-resources-form-grid agent-automation-resources-form-grid--3col">
        ...
        <label><span>运行目标说明</span><input class="form-input" name="goalTemplate" type="text" maxlength="2000" value="${agentEscapeAttr(config.goalTemplate)}" placeholder="例如：处理订单状态变化"></label>`;
}
...
function renderAgentAutomationResourceEditor(kind, item = null, draft = {}) {
    ...
    PivotSafeHtml.setHtml(editor, `
        ...
        <label><span>用途说明</span><input class="form-input" name="description" type="text" maxlength="500" value="${agentEscapeAttr(draft.description ?? item?.description ?? '')}" placeholder="说明凭据对应的系统和用途"></label>
