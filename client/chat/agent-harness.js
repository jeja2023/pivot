(function () {
    ...
    function renderAgentControlPlane() {
        ...
        if (channelsPanel) {
            const chStatusBadge = (s) => {
                if (s === 'active') return '<span class="agent-inbox-type-badge badge-run">● 活跃</span>';
                if (s === 'error') return '<span class="agent-inbox-type-badge badge-approval">✕ 异常</span>';
                return '<span class="agent-inbox-type-badge badge-event">○ 停用</span>';
            };
            const deliveryStatusBadge = (s) => {
                if (s === 'delivered') return '<span class="agent-inbox-type-badge badge-run">已送达</span>';
                if (s === 'dead_letter') return '<span class="agent-inbox-type-badge badge-approval">投递失败</span>';
                return '<span class="agent-inbox-type-badge badge-event">等待投递</span>';
            };
            const deliveryRows = Array.isArray(state.deliveries) ? state.deliveries : [];
            const deliveryRowsHtml = deliveryRows.slice(0, 20).map(item => {
                const subj = item.subject || item.eventType || '渠道投递';
                const errText = item.lastError || '—';
                const errAttr = item.lastError ? `title="${escapeAttr(item.lastError)}"` : '';
                const retryBtn = ['dead_letter', 'queued'].includes(item.status)
                    ? `<button type="button" class="btn-secondary btn-xs" data-agent-channel-delivery-retry="${escapeAttr(item.id)}">重投</button>`
                    : '—';
                return `<tr><td title="${escapeAttr(subj)}">${escape(shortText(subj, 36))}</td><td class="tc">${deliveryStatusBadge(item.status)}</td><td class="tc">${Number(item.attempts || 0)}</td><td ${errAttr}>${escape(shortText(errText, 60))}</td><td class="tc">${retryBtn}</td></tr>`;
            }).join('');
            const deliveryMarkup = deliveryRows.length
                ? `<section class="agent-channel-deliveries"><div class="agent-harness-subhead"><strong>投递状态</strong><span>任务完成与外部送达分开记录；失败可单独重投。</span></div><div class="aht-wrap"><table class="aht data-table"><thead><tr><th>主题</th><th class="col-status tc">状态</th><th class="col-attempts tc">尝试</th><th>最近错误</th><th class="col-actions tc">操作</th></tr></thead><tbody>${deliveryRowsHtml}</tbody></table></div></section>`
                : '<div class="agent-channel-delivery-empty">尚无渠道投递记录。</div>';
            const chListHtml = state.channels.length
                ? `<div class="aht-wrap"><table class="aht data-table"><thead><tr><th class="col-index tc">序号</th><th class="col-type">渠道类型</th><th>目标地址 / 用户标识</th><th class="col-cred">凭据引用</th><th class="col-channel-status tc">状态</th><th class="col-channel-actions tc">操作</th></tr></thead><tbody>${state.channels.map((ch, i) => `<tr><td class="tc font-mono">${i + 1}</td><td>${escape(ch.channelType)}</td><td title="${escapeAttr(ch.channelKey)}">${escape(ch.channelKey)}</td><td class="mono">${escape(ch.credentialRef || '—')}</td><td class="tc">${chStatusBadge(ch.status)}</td><td class="tc"><div class="aht-actions"><button type="button" class="btn-secondary btn-xs" data-agent-channel-test="${escapeAttr(ch.id)}">测试</button><button type="button" class="btn-secondary btn-xs" data-agent-channel-pair="${escapeAttr(ch.id)}">配对码</button><button type="button" class="btn-secondary btn-xs" data-agent-channel-sessions="${escapeAttr(ch.id)}">会话</button></div></td></tr>`).join('')}</tbody></table></div><div class="agent-channel-gateway-details">${personalExperience.renderGatewayDetails(state.channels)}</div>`
