(function () {
    ...
    function renderFeedback() {
        ...
        setMarkup(list, `<div class="aht-wrap"><table class="aht"><thead><tr><th style="width:56px" class="tc">序号</th><th style="width:110px" class="tc">运行 ID</th><th style="width:80px" class="tc">结论</th><th style="width:70px" class="tc">评分</th><th>修正意见</th><th style="width:110px" class="tc">时间</th></tr></thead><tbody>${pageFeedback.map((item, i) => `<tr><td class="tc font-mono">${startIndex + i + 1}</td><td class="tc mono" title="${escapeAttr(item.runId)}">${escape(shortText(item.runId, 10))}</td><td class="tc">${fbOutcomeBadge(item.outcome)}</td><td class="tc">${item.rating ? `${item.rating}/5` : '—'}</td><td title="${escapeAttr(item.correction || item.modifiedAnswer || '')}">${escape(shortText(item.correction || item.modifiedAnswer || '—', 80))}</td><td class="tc">${escape(formatDate(item.updatedAt))}</td></tr>`).join('')}</tbody></table></div>`);
        if (paginationContainer) {
            if (window.renderWorkspacePagination) {
