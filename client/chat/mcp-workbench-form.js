...
const MCP_CONFIG_HELPERS = {
    ...
    visualization: {
        title: '图表生成',
        steps: ['保存默认名称', '回到工具库查看工具', '在聊天或智能体中生成图表'],
        note: '普通用户通常先使用系统工具验证工具库是否可用；系统内置工具无需额外连接。'
    },
    report: {
        title: '报告编排',
        steps: ['保存默认名称', '回到工具库查看工具', '把摘要、表格、图表组合成报告'],
        note: '适合日报、周报和固定格式分析报告。'
    },
    documents: {
        title: '文档解析',
        steps: ['保存默认名称', '回到工具库查看工具', '让模型提取大纲、键值或分段'],
        note: '适合合同、制度、说明书等长文档的结构化处理。'
    },
    data: {
        title: '数据处理',
        steps: ['保存默认名称', '回到工具库查看工具', '让模型做筛选、汇总和字段标准化'],
        note: '适合表格行数据的清洗、分组统计和字段画像。'
    },
    format: {
        title: '格式转换',
        steps: ['保存默认名称', '回到工具库查看工具', '让模型在 JSON、Markdown 表格和文本之间转换'],
