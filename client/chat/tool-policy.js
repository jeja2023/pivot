...
const TOOL_POLICY_KNOWN_TITLES = {
    ...
    'models.list': '可用模型列表',
    'artifact.render': '文档渲染',
    'artifact.list_renditions': '渲染结果列表',
    'system.health': '系统健康',
    'system.modelRuntime': '模型运行状态',
    'rag.search': '知识库检索',
    'knowledge.list': '知识库文档列表',
...
const TOOL_POLICY_KNOWN_DESCRIPTIONS = {
    ...
    'data.group_summary': '对上游表格行按指定字段分组后计算一个或多个统计指标。',
    ...
    'db.group_count': '直接查询数据库表，按指定字段分组并统计每组数量，系统自动生成安全只读 SQL。',
    ...
    'workflow.template': '使用工作流变量拼接确定性文本，不调用模型、不执行代码。',
    'workflow.notify': '通过已配置的企业微信、飞书或钉钉渠道绑定排队发送通知，不接受裸 Webhook URL。',
