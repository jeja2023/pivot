...
8. 聊天输入区已收敛为默认自适应模式；一级菜单仅保留附件。裸“@”先选择知识库或工具，再通过中文分页列表浏览全部受限结果或使用中英文搜索显式指定；回答中的工具授权提示仍提供必要控制。
9. 工具展示层已与执行标识解耦：中文名称、英文别名和管理员自定义展示均不改变 `name`、`fullName`、Schema、白名单和执行链路。
