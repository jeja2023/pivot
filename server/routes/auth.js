...
function createAuthRouter({
    ...
    router.get('/auth/keys', authMiddleware, asyncHandler(async (req, res) => {
        const keys = db.prepare('SELECT id, name, key_preview, created_at, last_used_at, status, usage_tokens FROM api_keys WHERE user_id = ? ORDER BY created_at DESC').all(req.user.id);
