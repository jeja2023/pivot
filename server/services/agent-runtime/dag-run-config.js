...
async function inferDagRunGoal({ goal, title, workflowId, runMetadata = {}, dagSpec = null, user = {} }) {
    ...
    if (requestedWorkflowId && user?.id) {
        const workflow = await queryOne('SELECT name FROM agent_workflows WHERE id = ? AND user_id = ? AND deleted_at IS NULL', [
            requestedWorkflowId, user.id
        ]);
