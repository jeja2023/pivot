...
const { digestExternalValue } = require('./agent-channel-gateway');
...
async function receiveChannelInteraction(bindingId, payload = {}, headers = {}) {
    ...
    const identityPath = String((gatewayEnabled ? (gateway.identityPath || gateway.identity_path) : '') || config.identityPath || config.identity_path || '').trim();
    if (identityPath) {
        const identity = readByPath(payload, identityPath);
        if (gatewayEnabled) {
            // Gateway 审批只能由仍处于 paired 状态的外部身份触发；频道 ID/手机号
            // 或静态 allowlist 不能替代已签名的个人配对关系。
            const paired = await queryOne("SELECT id FROM agent_channel_pairings WHERE binding_id = ? AND external_identity_hash = ? AND status = 'paired'", [binding.id, digestExternalValue(signingSecret, binding.id, String(identity || ''))]);
            if (!paired) throw invalid('渠道审批身份尚未配对或已撤销。', 403, 'CHANNEL_INTERACTION_PAIRING_REQUIRED');
        } else {
    ...
    const identity = identityPath ? String(readByPath(payload, identityPath) || '') : '';
    const inserted = await queryOne(`INSERT INTO agent_channel_deliveries (binding_id, user_id, tenant_id, idempotency_key, event_type, subject, body, interaction, status, attempts, next_attempt_at, created_at, updated_at) VALUES (?, ?, ?, ?, 'channel.interaction', 'approval', ?, ?, 'queued', 0, ?, ?, ?) RETURNING id`, [binding.id, binding.user_id, binding.tenant_id || null, `inbound:${idempotencyKey}`, JSON.stringify({ requestId, decision, identityHash: identity ? digestExternalValue(signingSecret, binding.id, identity) : null, gatewayPaired: gatewayEnabled }), '渠道审批回调（身份已验证）', now, now, now]);
    try {
