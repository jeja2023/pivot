...
>
> **实施更新（v0.1.150）**：对话层已经默认启用智能自适应路由，并通过 @ 入口支持知识库和工具的可浏览选择。工作流数据工具采用明确边界：“数据库分组计数”直接查询数据表且仅统计数量；“表格分组汇总”处理上游表格行并支持多指标。这些展示名称沿用工具目录、权限策略、中文展示、参数 Schema 与审计链路，不改变动态工具发现的授权边界。
